...
void handleOpen() {
  ...
  gateIsOpen = true;
  closeGateTicker.once(4, handleAutoClose); // Mengatur timer untuk menutup gerbang setelah 4 detik
...
void loop() {
  ...
  if (digitalRead(leftIR) == LOW || digitalRead(rightIR) == LOW) {
    if (!objectDetected) {
      ...
      turnOnRGBWhiteOther(); // Menyalakan lampu RGB lain menjadi putih saat ada objek terdeteksi
    }
  } else {
    if (objectDetected) {
      objectDetected = false;
      digitalWrite(ledPin, LOW); // Mematikan LED saat tidak ada objek terdeteksi
      turnOffRGBOther(); // Mematikan lampu RGB lain saat tidak ada objek terdeteksi
    }
  }
}
